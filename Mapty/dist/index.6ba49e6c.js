...
class App {
    ...
    _loadMap(position) {
        ...
        this.#map = L.map('map').setView(coords3, this.#mapZoomLevel);
    ...
    _newWorkOut(e) {
        ...
        const distance3 = +inputDistance.value;
        const duration3 = +inputDuration.value;
        const { lat , lng  } = this.#mapEvent.latlng;
        let workout;
        // If workout running, create running obj
        if (type === "running") {
            const cadence1 = +inputCadence.value;
            // Check if data is valid
            if (!validInputs(distance3, duration3, cadence1) || !allPositive(distance3, duration3, cadence1)) return alert("Inputs have to be positive numbers");
            workout = new Running([
                lat,
                lng
            ], distance3, duration3, cadence1);
        }
        // If workout cycling, create cycling obj
        if (type === "cycling") {
            const elevation = +inputElevation.value;
            // Check if data is valid
            if (!validInputs(distance3, duration3, elevation) || !allPositive(distance3, duration3)) return alert("Inputs have to be positive numbers");
            workout = new Cycling([
                lat,
                lng
            ], distance3, duration3, elevation);
        }
